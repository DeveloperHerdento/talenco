...
import { HERO_BG_ANIMATION_MS } from "@/lib/constants/animation";
import type { Dictionary } from "@/lib/i18n/dictionary";
import type { Locale } from "@/lib/i18n/locales";

const CONTENT_START_MS = HERO_BG_ANIMATION_MS + 150;
  ...
  return (
    ...
    >
      <div className="absolute inset-2 overflow-hidden rounded-[20px] md:inset-3">
        <div
          className={`h-full w-full ${bgLoaded ? "animate-fade-up" : "opacity-0"}`}
          style={bgLoaded ? { animationDuration: `${HERO_BG_ANIMATION_MS}ms` } : undefined}
        >
          <Image
        ...
        {bgLoaded && (
          <div className="flex max-w-5xl flex-col gap-12">
            <h1 className="flex flex-col text-6xl leading-tight font-extrabold tracking-normal text-white sm:text-5xl md:text-6xl lg:text-7xl lg:leading-tight xl:text-[96px] xl:leading-tight">
              <FadeUp as="span" className="block" delayMs={CONTENT_START_MS + 0} durationMs={500}>
                <RichText text={dict.titleLine1} />
              </FadeUp>
              <FadeUp as="span" className="block" delayMs={CONTENT_START_MS + 120} durationMs={500}>
            ...
            <p className="text-base leading-normal text-white sm:text-lg md:text-xl lg:text-2xl">
              {dict.paragraph.map((line, index) => (
                <FadeUp
                  ...
                  as="span"
                  className="block"
                  delayMs={CONTENT_START_MS + 300 + index * 60}
                  durationMs={350}
                >
                  {line}
                </FadeUp>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <FadeUp className="inline-block" delayMs={CONTENT_START_MS + 500} durationMs={400}>
                <a href={`/${locale}/register`}>
                  <Button variant="primary" size="lg">{dict.ctaPrimary}</Button>
                </a>
              </FadeUp>
              <FadeUp className="inline-block" delayMs={CONTENT_START_MS + 580} durationMs={400}>
